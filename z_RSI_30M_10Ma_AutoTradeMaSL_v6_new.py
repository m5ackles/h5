...
def get_ma12(ticker):
    """12일 이동 평균선 조회"""
    df = pyupbit.get_ohlcv(ticker, interval="minute15", count=12)
...
while True:
    try:
        ...
        ma12 = get_ma12(coin_title)
        ma30 = get_ma30(coin_title)
        current_price = get_current_price(coin_title)


        # 12일선 위에 있으며, 변동성돌파시 매수,  또는 RSI가 28찍고 30이상이면 매수~~~~~~~~~~~~~~
        if target_price < current_price and ma12 < current_price or now_rsi == 28:
            lower28[i] = True
